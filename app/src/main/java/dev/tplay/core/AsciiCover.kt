...
private const val GLYPH_SIZE = 20f
...
private fun monoTypeface(context: Context): Typeface =
    cachedMonoTypeface ?: ResourcesCompat.getFont(context, R.font.jetbrains_mono)
        ?: Typeface.MONOSPACE.also { cachedMonoTypeface = it }
...
private val Paint.cellAdvance: Float get() = measureText("M")
private val Paint.cellAspect: Float get() = cellAdvance / textSize

private fun gammaLift(channel: Int): Int {
    val lifted = 255f * (channel / 255f).pow(0.7f)
...
private fun renderAsciiGrid(
    ...
    val height = (rows * paint.textSize).roundToInt()
    ...
    for (y in 0 until rows) {
        for (x in 0 until cols) {
            ...
            glyph[0] = symbol
            val textX = x * advance
            val baselineY = (y + 1) * paint.textSize - paint.textSize * BASELINE_SHIFT
...
fun Bitmap.toAsciiBitmap(context: Context, cols: Int = 88): Bitmap {
    ...
    return renderAsciiGrid(paint, cols, rows) { x, y, cellPaint ->
        ...
        if (symbol != ' ') cellPaint.color = AndroidColor.rgb(red, green, blue)
        symbol
    }
}

fun generateAsciiPlaceholder(context: Context, seed: Long, cols: Int = 88): Bitmap {
    val paint = asciiPaint(context)
    ...
    return renderAsciiGrid(paint, cols, rows) { x, y, cellPaint ->
        val v = (sin(x * f1 + p1) + sin(y * f2 + p2) + sin((x + y) * f3 + p3) + 3f) / 6f
...
fun AsciiCover(
    ...
    if (cover == null) {
        Canvas(modifier) {
            drawRect(Color(0xFF0E0F10))
        }
        return
    }
    val aspect = cover.width.toFloat() / cover.height
    if (playing && wave) {
        WaveCover(cover, aspect, modifier)
    } else {
